...
#define MODBUS_INFO_PRINTF_SET 1
...
uint8_t MBR_ReadParam_03H(modbus_rtu_t *mbr, uint8_t _addr, uint16_t _reg, uint16_t _num)
{
	...
	if (mbr->ack == 0)
	{
		MODBUS_ERROR_PRINTF("Lost reply,device id:0x%x,Register address:0x%x,cmd:03 \r\n", _addr, _reg);
		retval = 0;
	}
	else
		retval = 1;
...
uint8_t MBR_WriteParam_06H(modbus_rtu_t *mbr, uint8_t _addr, uint16_t _reg, uint16_t _value)
{
	...
	if (mbr->ack == 0)
	{
		MODBUS_ERROR_PRINTF("Lost reply, device id:0x%x, Register address:0x%x, cmd:06 \r\n", _addr, _reg);
		retval = 0;
	}
	else
		retval = 1;
...
uint8_t MBR_WriteParam_10H(modbus_rtu_t *mbr, uint8_t _addr, uint16_t _reg, uint8_t _num, uint8_t *_buf)
{
	...
	if (mbr->ack == 0)
	{
		MODBUS_ERROR_PRINTF("Lost reply, device id:0x%x, Register address:0x%x, cmd:10 \r\n", _addr, _reg);
		retval = 0;
	}
	else
	{
		//				MODBUS_INFO_PRINTF("success! device id:0x%x, Register address:0x%x, cmd:10 \r\n",_addr,_reg);
		retval = 1;
	}
	...
	// log_i("_buf[0] = %d _buf[1] = %d _buf[2] = %d _buf[3] = %d",_buf[0],_buf[1],_buf[2],_buf[3]);
...
void MBR_Read_03H_Block(modbus_rtu_t *mbr, uint8_t _addr, uint16_t _reg, uint16_t _num)
{
	...
	{
		Modbus_Delay_MS(10);
		MODBUS_INFO_PRINTF("MBR_Read_03H_Block, device id:0x%x, Register address:0x%x, cmd:03 \r\n", _addr, _reg);
		Modbus_Delay_MS(10);
		mbr->ack = 1;
...
void MBR_Write_06H_Block(modbus_rtu_t *mbr, uint8_t _addr, uint16_t _reg, uint16_t _value)
{
	...
	{
		Modbus_Delay_MS(10);
		MODBUS_INFO_PRINTF("MBR_Read_06H_Block, device id:0x%x, Register address:0x%x, cmd:06, value:%d \r\n", _addr, _reg, _value);
		Modbus_Delay_MS(15);
		mbr->ack = 1;
...
void MBR_Write_10H_Block(modbus_rtu_t *mbr, uint8_t _addr, uint16_t _reg, uint8_t _num, uint8_t *_buf)
{
	...
	{
		Modbus_Delay_MS(10);
		MODBUS_INFO_PRINTF("MBR_Read_10H_Block, device id:0x%x, Register address:0x%x, cmd:10 \r\n", _addr, _reg);
		Modbus_Delay_MS(15);
		mbr->ack = 1;
