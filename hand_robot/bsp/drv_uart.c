...
extern UART_HandleTypeDef huart6;   //e28
...
extern UART_HandleTypeDef huart2;   //modbus1
extern DMA_HandleTypeDef hdma_usart2_rx;
extern DMA_HandleTypeDef hdma_usart2_tx;
uint8_t usart2_rx_buff[USART2_RX_BUFFER_SIZE];
...
extern UART_HandleTypeDef huart3;   //modbus2
...
extern UART_HandleTypeDef huart1;   //modbus3
...
void usart1_idle_callback(void)
{
    if (__HAL_UART_GET_FLAG(&huart1, UART_FLAG_IDLE))
    {
        ...
        usart1_manage_obj.call_back_f(usart1_manage_obj.rx_buffer, USART1_RX_BUFFER_SIZE - __HAL_DMA_GET_COUNTER(huart1.hdmarx));
...
void usart1_manage_init(void)
{
    ...
    UART_Receive_DMA_No_IT(&huart1, usart1_rx_buff, USART1_RX_BUFFER_SIZE);
...
void usart1_send_before(void)
{
    HAL_GPIO_WritePin(USART1_TXEN_GPIO_Port, USART1_TXEN_Pin, GPIO_PIN_SET);
}

void usart1_send_over(void)
{
    HAL_GPIO_WritePin(USART1_TXEN_GPIO_Port, USART1_TXEN_Pin, GPIO_PIN_RESET);
}
...
int32_t usart_transmit(usart_manage_obj_t *m_obj, uint8_t *buf, uint16_t len)
{
    ...
    {
//		if(len > m_obj->tx_fifo.free_num && len < m_obj->tx_fifo_size)
//		{
//			
//		}
